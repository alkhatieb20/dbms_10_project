...
\textbf{Repository:} \url{https://github.com/<user>/exam-generation}
\end{infobox}
...
\textbf{Paketierung.} Die Anwendung wird mit PyInstaller eingefroren und mit \texttt{fpm} in ein Debian-Paket verpackt, sodass sie auf dem Vorlesungsserver nativ installiert werden kann:

\begin{lstlisting}[caption={Bauen und Installieren der .deb-Datei}]
...
\begin{lstlisting}[caption={docker-compose.yml (Auszug)}]
services:
  postgres:
    image: postgres:16
    environment:
...
\begin{enumerate}[leftmargin=1.6em, topsep=2pt]
  ...
  \item Starten Sie \texttt{exam-frontend} (über /usr/bin/exam-frontend/exam-frontend) und verbinden Sie sich über den Dialog.
\end{enumerate}
