...
class ConnectionDialog(tk.Toplevel):
    def __init__(self, parent: tk.Tk):
        ...
        tk.Entry(self, textvariable=self._url, width=38).grid(row=0, column=1, padx=10)

        tk.Label(self, text="X-API-Key:").grid(row=1, column=0, padx=14, pady=10, sticky=tk.E)
        
        tk.Entry(self, textvariable=self._token, width=38, show="*").grid(row=1, column=1, padx=10)

        tk.Button(self, text="Connect", command=self._confirm).grid(row=2, column=0, columnspan=2, pady=12)
